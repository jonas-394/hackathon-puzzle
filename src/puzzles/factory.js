...
const TIER1 = ['numberSeq', 'letterSeq'];
const TIER2 = ['numberSeq', 'letterSeq', 'analogy', 'wordAnalogy'];
const TIER3 = ['numberSeq', 'letterSeq', 'analogy', 'wordAnalogy', 'oddOneOut', 'shapeSeq'];
const TIER4 = ['numberSeq', 'letterSeq', 'analogy', 'wordAnalogy', 'oddOneOut', 'shapeSeq'];
function tierForLevel(level) {
    if (level <= 3)
        return { pool: TIER1, showTypeLabel: true, freeHints: 1 };
    if (level <= 6)
        return { pool: TIER2, showTypeLabel: true, freeHints: 0 };
    if (level <= 10)
        return { pool: TIER3, showTypeLabel: true, freeHints: 0 };
...
export async function createPuzzle(level, rng) {
