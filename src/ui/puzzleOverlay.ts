import type { Puzzle, Door } from '../types';
...
export class PuzzleOverlay {
  ...
  private onResult!: (result: PuzzleResult) => void;
  ...
  private render(tokens: number, statusMsg: string, statusClass = ''): void {
    ...
    this.el.querySelector('#puzzle-input')?.addEventListener('keydown', (e) => {
      if ((e as KeyboardEvent).key === 'Enter') this.submitAnswer(tokens);
    });
    ...
    if (isLocked) this.startLockoutCountdown(tokens);
  }

  private async submitAnswer(tokens: number): Promise<void> {
    const door = this.currentDoor;
    const now = Date.now();
    if (door.lockedUntil > now) return;

    const input = this.el.querySelector('#puzzle-input') as HTMLInputElement | null;
    if (!input) return;
  ...
  private revealHint(tokens: number): void {
    if (this.hintsRevealed >= this.currentPuzzle.hints.length) return;
    const isFree = this.hintsRevealed < this.currentPuzzle.freeHints;
    if (!isFree && tokens < 1) return;
  ...
  private shakeCard(isError: boolean): void {
    const card = this.el.querySelector('#puzzle-card') as HTMLElement | null;
    if (!card) return;
    if (isError) {
  ...
  private startLockoutCountdown(tokens: number): void {
    if (this.lockoutTimer) clearTimeout(this.lockoutTimer);
